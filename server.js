...
app.get('/player_info/post', (req, res) => {
    ...
    var Player_Info = 'select * from football_players where id=' + req.query.player_choice + ';';
    var Games_Played = 'select count(*) from football_games where ' + req.query.player_choice + ' = any(players);';

    if (req.query.player_choice === "Select Player") {
        db.task('thePage', task => {
            return task.batch([
                task.any(Players)
            ])
